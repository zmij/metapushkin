...
private:
    ...
    test(_check<void (_fallback::*)(), &C::operator()>*);

public:
    static const bool value = ::std::is_same<decltype(test<_derived>(0)), ::std::true_type>::value;
};
...
struct is_callable_object
    : ::std::conditional<::std::is_class<T>::value, detail::has_call_operator<T>,
...
template <typename T>
struct function_traits<T> : ::std::conditional<is_callable_object<T>::value,
                                               call_operator_traits<T>, not_a_function<T>>::type {};

template <typename Func>
struct is_func_void
    : ::std::conditional<::std::is_same<typename function_traits<Func>::result_type, void>::value,
