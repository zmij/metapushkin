...
struct insert_type_pair<type_map_base<type_tuple<T...>, type_tuple<U...>>, K, V>
    : ::std::conditional<contains<K, T...>::value, key_exists<K, K>,
                         type_map_base<type_tuple<K, T...>, type_tuple<V, U...>>>::type {};
...
struct type_map<type_tuple<K, T...>, type_tuple<V, Y...>> {
    using unique_keys = insert_type_pair<type_map_base<type_tuple<T...>, type_tuple<Y...>>, K, V>;
    using key_types   = typename unique_keys::key_types;
    using value_types = typename unique_keys::value_types;

    static_assert(key_types::size == value_types::size, "Incorrect size of type_map");

    static constexpr ::std::size_t size = key_types::size;

    template <::std::size_t N>
    using type
        = type_pair<typename key_types::template type<N>, typename value_types::template type<N>>;
};
