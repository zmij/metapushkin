...
struct contains<char_range<Min, Max>, C> : ::std::integral_constant<bool, (Min <= C && C <= Max)> {
};
//@}
 ...
 */
template <typename IndexTuple, template <char> class Predicate, typename V, V ifTrue,
          V ifFalse = V{}>
struct build_sort_table;

template <::std::size_t... Indexes, template <char> class Predicate, typename V, V ifTrue,
          V ifFalse>
struct build_sort_table<::std::integer_sequence<::std::size_t, Indexes...>, Predicate, V, ifTrue,
                        ifFalse> {

    using size = ::std::integral_constant<::std::size_t, sizeof...(Indexes)>;
    using type
        = ::std::integer_sequence<V, (Predicate<(char)Indexes>::value ? ifTrue : ifFalse)...>;
};
...
struct make_char_sort_table<char_sequence<Chars...>, Index> {
    ...
    using type      = typename build_sort_table<Index, predicate, bool, true, false>::type;
};
...
struct sort_helper<N, ::std::integer_sequence<bool, V, Vals...>, Chars...> {
    using type = typename ::std::conditional<
        V, sort_helper<N - 1, ::std::integer_sequence<bool, Vals...>, char(N), Chars...>,
        sort_helper<N - 1, ::std::integer_sequence<bool, Vals...>, Chars...>>::type::type;
};

template <unsigned char N, bool V, char... Chars>
struct sort_helper<N, ::std::integer_sequence<bool, V>, Chars...> {
    using type = typename ::std::conditional<V, char_sequence<char(N), Chars...>,
                                             char_sequence<Chars...>>::type;
};
...
struct overlapping_range_join<A, B, C, D, false> {
    using type =
        typename unique_sort<typename join<typename unwrap_char_range<A, B>::type,
                                           typename unwrap_char_range<C, D>::type>::type>::type;
};
...
struct including_range_join<A, B, C, false> {
    using type = typename unique_sort<typename join<typename unwrap_char_range<A, B>::type,
                                                    ::std::integral_constant<char, C>>::type>::type;
};
...
struct join<char_range<A, B>, char_range<C, D>>
    : detail::overlapping_range_join<
          A, B, C, D, chars::overlaps<char_range<A, B>, char_range<C, D>>::type::value> {};

template <char A, char B, char C>
struct join<char_range<A, B>, ::std::integral_constant<char, C>>
    : detail::including_range_join<A, B, C, chars::contains<char_range<A, B>, C>::value> {};
...
struct join<char_range<A, B>, char_sequence<Chars...>> {
    using type = typename unique_sort<typename join<typename detail::unwrap_char_range<A, B>::type,
                                                    char_sequence<Chars...>>::type>::type;
};
...
using make_char_sequence =
    typename detail::make_char_sequence_impl<Str,
                                             ::std::make_index_sequence<detail::strlen(Str)>>::type;

template <char A, char B>
struct make_char_literal<char_range<A, B>> {
    using type = typename make_char_literal<typename detail::unwrap_char_range<A, B>::type>::type;
};
...
template <char const* Str>
using make_char_literal_s =
    typename detail::make_char_literal_impl<Str,
                                            ::std::make_index_sequence<detail::strlen(Str)>>::type;
