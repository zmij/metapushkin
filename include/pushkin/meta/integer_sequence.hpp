...
struct join<T, U, V...> : join<join_t<T, U>, V...> {};
...
struct split_range_helper {
    using type = typename join<typename unwrap_range_impl<T, Min, Min + template_depth - 1>::type,
                               typename unwrap_range<T, Min + template_depth, Max>::type>::type;
};

template <typename T, T Min, T Max>
struct unwrap_range
    : ::std::conditional<(Max - Min < template_depth), unwrap_range_impl<T, Min, Max>,
                         split_range_helper<T, Min, Max>>::type {};

template <typename T, T Min, T Max>
struct reverse_split_range_helper {
    using type = typename join<
        typename reverse_unwrap_range<T, Min + template_depth, Max>::type,
        typename reverse_unwrap_range_impl<T, Min, Min + template_depth - 1>::type>::type;
};
template <typename T, T Min, T Max>
struct reverse_unwrap_range
    : ::std::conditional<(Max - Min < template_depth), reverse_unwrap_range_impl<T, Min, Max>,
                         reverse_split_range_helper<T, Min, Max>>::type {};
...
struct find_index_if_impl<Head, Predicate, T>
    : ::std::conditional<Predicate<T>::value, ::std::index_sequence<Head>,
                         ::std::index_sequence<>> {};
