...
enum class range_direction_type { empty, ascending, descending };
...
struct range {
    ...
    inline static constexpr range_direction_type direction
        = (begin < end
               ? range_direction_type::ascending
               : (end < begin ? range_direction_type::descending : range_direction_type::empty));
...
template <typename T>
using max_range = range<T, std::numeric_limits<T>::min(), std::numeric_limits<T>::max(), true>;
...
using max_value_range = value_range<Key, Value, std::numeric_limits<Key>::min(),
...
struct reverse {
    ...
    using type =
        typename original_type ::template range_type<original_type::end, original_type::begin>;
};
template <typename T>
...
struct align {
    ...
    using type = std::conditional_t<direction == range_direction_type::empty
                                        || original_type::direction == direction,
                                    original_type, reverse_t<original_type>>;
};
template <range_direction_type Direction, typename T>
...
normalize()
{
    ...
    if constexpr (aligned_type::include_end) {
        constexpr bool can_overflow = range_end_v<T> == std::numeric_limits<bounds_type>::max();
        if constexpr (!can_overflow) {
            return typename aligned_type::template range_type<aligned_type::begin,
                                                              aligned_type::end + 1, false>{};
        } else {
            return aligned_type{};
        }
    } else
...
template <typename T, typename U>
struct has_bounds_of_type : decltype(detail::has_bounds_of_type<T, U>()) {};
template <typename T, typename U>
...
template <typename T, typename U>
struct has_value_of_type : decltype(detail::has_value_of_type<T, U>()) {};
template <typename T, typename U>
...
struct are_same_range_type<range<T, LBegin, LEnd, LInc>, range<T, RBegin, REnd, RInc>>
    : std::true_type {};
template <typename Key, typename Value, Key LBegin, Key LEnd, Value LVal, bool LInc, Key RBegin,
          Key REnd, Value RVal, bool RInc>
struct are_same_range_type<value_range<Key, Value, LBegin, LEnd, LVal, LInc>,
...
have_same_bounds_type()
{
    static_assert(is_range_v<LHS>, "Type must be a range");
    static_assert(is_range_v<RHS>, "Type must be a range");
    return std::is_same<range_bounds_type<LHS>, range_bounds_type<RHS>>::value;
}
...
have_same_value_type()
{
    ...
    return std::is_same<range_value_type<LHS>, range_value_type<RHS>>::value;
}
...
have_same_value()
{
    static_assert((have_same_value_type_v<LHS, RHS>), "Ranges must have the same value type");
    return range_value_v<LHS> == range_value_v<RHS>;
}

}    // namespace detail
template <typename LHS, typename RHS>
struct have_same_value : std::integral_constant<bool, detail::have_same_value<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
same_direction()
{
    static_assert((have_same_bounds_type_v<LHS, RHS>), "Ranges must have the same bounds type");
    return LHS::direction == RHS::direction;
}

}    // namespace detail
template <typename LHS, typename RHS>
struct are_same_direction : std::integral_constant<bool, detail::same_direction<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
template <typename LHS, typename RHS>
struct begin_compare : std::integral_constant<bool, detail::begin_compare<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
template <typename LHS, typename RHS>
struct end_compare : std::integral_constant<bool, detail::end_compare<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
template <typename LHS, typename RHS>
struct before_compare : std::integral_constant<bool, detail::is_before<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
template <typename LHS, typename RHS>
struct overlaps : std::integral_constant<bool, detail::overlaps<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
contains()
{
    ...
    using lhs_aligned = align_t<range_direction_type::ascending, LHS>;
    using rhs_aligned = align_t<range_direction_type::ascending, RHS>;
    return lhs_aligned::begin <= rhs_aligned::begin && rhs_aligned::end <= lhs_aligned::end;
}

}    // namespace detail

template <typename LHS, typename RHS>
struct contains : std::integral_constant<bool, detail::contains<LHS, RHS>()> {};
template <typename LHS, typename RHS>
...
struct contained {
    using type = std::conditional_t<
        overlaps_v<LHS, RHS>,
        std::conditional_t<contains_v<LHS, RHS>, RHS,
                           std::conditional_t<contains_v<RHS, LHS>, LHS, void>>,
        void>;
};
template <typename LHS, typename RHS>
...
intersect()
{
    static_assert((are_same_range_type_v<LHS, RHS>), "Ranges must have the same type");
    ...
    if constexpr (overlaps_v<LHS, RHS>) {
        ...
        using intersection =
            typename right_begin::template range_type<right_begin::begin, left_end::end,
                                                      left_end::include_end>;
...
make_range_sequence()
{
    static_assert(is_value_range_v<LHS>, "Type must be a value range");
    static_assert(is_value_range_v<RHS>, "Type must be a value range");
    static_assert((are_same_range_type_v<LHS, RHS>), "Types must be of same type");

    using bounds_type = range_bounds_type<LHS>;
    using value_type  = Value;
    using lhs_norm    = normalize_t<LHS>;
    using rhs_norm    = normalize_t<RHS>;
    using begin_order = stable_order<meta::range::begin_compare, lhs_norm, rhs_norm>;
    using end_order   = stable_order<meta::range::end_compare, lhs_norm, rhs_norm>;
    if constexpr (lhs_norm::value == rhs_norm::value) {
        return type_tuple<
            value_range<bounds_type, value_type, begin_order::first::begin, end_order::second::end,
        ...
        using end_rev_order         = stable_order<meta::range::end_compare, rhs_norm, lhs_norm>;
        constexpr bool ends_equal   = (end_order::original_order && end_rev_order::original_order);

        using first = std::conditional_t<
            begins_equal, void,
            value_range<bounds_type, value_type, begin_order::first::begin,
                        begin_order::second::begin, begin_order::first::value, false>>;
        using middle = value_range<bounds_type, value_type, begin_order::second::begin,
                                   end_order::first::end, begin_order::second::value,
                                   end_order::first::include_end>;
        using last   = std::conditional_t<
            ends_equal, void,
            value_range<bounds_type, value_type, end_order::first::end, end_order::second::end,
        ...
        return type_tuple<
            lhs_norm,
            value_range<bounds_type, value_type, lhs_norm::end, rhs_norm::begin, Default, false>,
            rhs_norm>{};
    } else {
...
struct add_range_to_end {
    ...
    using type        = combine_t<head, make_adjacent_ranges_t<Value, Default, tail, next>>;
};

template <typename Value, Value Default, typename RangeTuple, typename Range, typename Overlap,
...
struct replace_overlap<Value, Default, RangeTuple, Range, Overlap, 1> {
    ...
    using type = stable_sort_t<
        ...
        combine_t<overlap_removed,
                  make_adjacent_ranges_t<Value, Default, combine_t<overlap, next>>>>;
};
...
struct merge_ranges<Value, Default, RangeTuple, Range, Rest...> {
    ...
    using merged =
        typename replace_overlap<Value, Default, range_tuple, next, overlap, overlap::size>::type;
...
struct build_range_values {
    ...
    static constexpr type value{range_value_v<T>...};
};
template <typename... T>
...
template <typename Key, typename Value, Value Default, typename... Ranges>
struct range_map {
    using key_type                            = Key;
    using value_type                          = Value;
    static constexpr value_type default_value = Default;

    static_assert((all_match_v<is_value_range, Ranges...>), "All ranges must be value ranges");
    ...
    using default_value_range = max_value_range<key_type, value_type, default_value>;
    using ranges              = detail::build_adjacent_ranges<
        value_type, default_value,
        combine_t<default_value_range,
                  stable_sort_t<begin_compare,
                                transform_t<normalize_t, remove_if_t<is_empty, Ranges...>>>>>;
    ...
    lower_bound(key_type const& k)
    {
        ...
        if (right > range_starts.begin() && k >= *(right - 1)) {
            return (right - 1);
        }
