...
struct is_empty<type_tuple<T...>>
    : ::std::conditional<(sizeof...(T) > 0), ::std::false_type, ::std::true_type>::type {};
template <typename T>
...
template <typename T, T V, T... Values>
struct back<::std::integer_sequence<T, V, Values...>>
    : back<::std::integer_sequence<T, Values...>> {};
template <typename T>
...
struct combine<type_tuple<T...>, U, Y...> : combine<type_tuple<T..., U>, Y...> {};

template <typename T, typename... U, typename... Y>
struct combine<T, type_tuple<U...>, Y...> : combine<type_tuple<T, U...>, Y...> {};
...
struct slice_impl<::std::index_sequence<Indexes...>, type_tuple<T...>> {
    ...
    static_assert(front_t<index_tuple>::value < sizeof...(T),
                  "Start index of slice is out of range");
...
template <typename T, typename... Y>
struct insert_type
    : ::std::conditional_t<contains_v<T, Y...>, noop<T, Y...>,
                           ::std::conditional_t<::std::is_same<T, void>::value, noop<T, Y...>,
...
template <typename... T, typename... Y>
struct unique<type_tuple<T...>, type_tuple<Y...>> : unique<T..., Y...> {};

template <typename... T, typename... Y>
struct unique<unique<T...>, unique<Y...>> : unique<T..., Y...> {};
//@}
...
struct all_match<Predicate, T, Y...>
    : ::std::conditional<Predicate<T>::value, all_match<Predicate, Y...>, ::std::false_type>::type {
};

template <template <typename> class Predicate, typename T>
struct all_match<Predicate, T>
    : ::std::conditional<Predicate<T>::value, ::std::true_type, ::std::false_type>::type {};

template <template <typename> class Predicate>
struct all_match<Predicate> : ::std::false_type {};
...
struct any_match<Predicate, T, Y...>
    : ::std::conditional<Predicate<T>::value, ::std::true_type, any_match<Predicate, Y...>>::type {
};
...
template <template <typename> class Predicate, typename... T>
struct any_match<Predicate, type_tuple<T...>> : any_match<Predicate, T...> {};
//@}
...
template <template <typename> class Predicate, typename... T>
struct transform<Predicate, type_tuple<T...>> : transform<Predicate, T...> {};
...
template <template <typename, typename> class Compare, typename T1, typename... TN, typename Y1,
          typename... YN>
struct merge_sorted<Compare, type_tuple<T1, TN...>, type_tuple<Y1, YN...>>
    : ::std::conditional_t<
          stable_order<Compare, T1, Y1>::original_order,
          combine<type_tuple<T1>,
                  merge_sorted_t<Compare, type_tuple<TN...>, type_tuple<Y1, YN...>>>,
          combine<type_tuple<Y1>,
                  merge_sorted_t<Compare, type_tuple<T1, TN...>, type_tuple<YN...>>>> {};
//@}
...
struct stable_sort {
    ...
    using type = merge_sorted_t<Compare, stable_sort_t<Compare, first_half>,
                                stable_sort_t<Compare, second_half>>;
};
